    ...
    context "when indexes are created" do
      ...
      context "when slug is not scoped by a reference association" do
        it "defines an index on the slug" do
          Book.index_specifications.detect { |spec| spec.key == { :_slugs => 1 } }.should_not be_nil
        end

        it "defines a unique index" do
          Book.index_specifications.detect { |spec| spec.key == { :_slugs => 1 } }.options[:unique].should be_true
        end
      end

      context "when slug is scoped by a reference association" do
        it "does not define an index on the slug" do
          Author.index_specifications.detect { |spec| spec.key == { :_slugs => 1 } }.should be_nil
        end
      end
      ...
      context "for subclass scope" do
        context "when slug is not scoped by a reference association" do
          it "defines an index on the slug" do
            BookPolymorphic.index_specifications.detect { |spec| spec.key == { :_type => 1, :_slugs => 1 } }.should_not be_nil
          end

          it "defines a unique index" do
            BookPolymorphic.index_specifications.detect { |spec| spec.key == { :_type => 1, :_slugs => 1 } }.options[:unique].should be_false
          end
        end

        context "when slug is scoped by a reference association" do
          it "does not define an index on the slug" do
            AuthorPolymorphic.index_specifications.detect { |spec| spec.key == { :_type => 1, :_slugs => 1 } }.should be_nil
          end
        end
